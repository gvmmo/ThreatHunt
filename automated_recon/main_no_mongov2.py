...
class AutomatedRecon:
    def __init__(self, target: str, target_type: str, scan_type: str = "passive", tools: List[str] = None):
        ...
        print(f"[DEBUG] Shodan API Key loaded: {'Yes' if self.shodan_api_key else 'No'}")
    ...
    def _get_domain_ips(self, domain: str) -> List[str]:
        """Get IPv4 addresses for a domain using DNS resolution."""
        try:
            import socket
            ips = []
            try:
                print(f"\n[DEBUG] Resolving IPs for {domain}")
                ...
                for item in info:
                    ...
                    if ':' not in ip and ip not in ips:
                        ips.append(ip)
                print(f"[DEBUG] Found IPs for {domain}: {ips}")
            except socket.gaierror as e:
                print(f"[DEBUG] DNS resolution failed for {domain}: {str(e)}")
            return ips
        except Exception as e:
            print(f"[DEBUG] Error getting IPs for {domain}: {str(e)}")
            return []
            ...
            for protocol in ['http', 'https']:
                try:
                    ...
                    if response.status_code < 400:  # Any status code less than 400 means the server responded
                        status = 'up'
                        print(f"[DEBUG] {url} is up (status code: {response.status_code})")
                        break
                except requests.exceptions.RequestException as e:
                    print(f"[DEBUG] {url} failed: {str(e)}")
                    continue
            
            return ips, status
            
        except Exception as e:
            print(f"[DEBUG] Error checking status for {domain}: {str(e)}")
            return [], 'down'
    ...
    def _process_domain_asset(self, asset: Dict[str, Any], is_initial_target: bool = False) -> None:
        ...
        if is_initial_target:
            # Comment out Subfinder temporarily
            '''
            if 'subfinder' in self.tools:
                subfinder_results = self.subfinder.run_subfinder()
                if subfinder_results:
                    for sub_data in subfinder_results:
                        subdomain = sub_data.get('host', '')
                        if subdomain and subdomain not in self.results['assets']:
                            sub_asset = self._create_asset(subdomain, 'domain', 'subfinder', domain)
                            self._add_relationship(domain, subdomain, 'has_subdomain', 'Subfinder discovery')
                            
                            # Process subdomain but don't run subfinder again
                            self._process_domain_asset(sub_asset)
            '''
            
            # Use VirusTotal for subdomain discovery
            if 'virustotal' in self.tools:
                print(f"\n[DEBUG] Getting subdomains for {domain} from VirusTotal")
                subdomains_data = self.virustotal.get_subdomains(domain)
                
                if subdomains_data and 'data' in subdomains_data:
                    print(f"[DEBUG] Found {len(subdomains_data['data'])} subdomains")
                    for sub_data in subdomains_data['data']:
                        if 'id' in sub_data:
                            subdomain = sub_data['id']
                            print(f"[DEBUG] Found subdomain: {subdomain}")
                            if subdomain and subdomain not in self.results['assets']:
                                print(f"[DEBUG] Creating new asset for subdomain: {subdomain}")
                                ...
                                print(f"[DEBUG] Processing subdomain with tools: {subdomain}")
                                self._process_domain_asset(sub_asset, is_initial_target=False)
                        else:
                            print(f"[DEBUG] No 'id' found in subdomain data: {sub_data}")
                else:
                    print(f"[DEBUG] No subdomains found in VirusTotal response")
        ...
        '''
        if 'shodan' in self.tools and (is_initial_target or asset['discovery_context']['source'] == 'subfinder')
            shodan_results = self.shodan.scan_domain(domain)
            if shodan_results:
        ...
        try:
            ...
            while True:
                ...
                for asset_id in new_assets:
                    ...
                    print(f"Processing discovered asset: {asset_id} ({asset['type']})")
...
def main():
    ...
    target_type = "domain"
    scan_type = "passive"
    tools = ['shodan', 'virustotal', 'wappalyzer',]  # Example subset of tools
